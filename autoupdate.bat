...
::                             |_|
::
:: Automatic RDP Wrapper installer and updater v.1.3       IHKYoung(2024-12-17)
:: ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
:: Options:
...
::   contributors: "sebaxakerhtc, asmtron, affinityv"
...
:: ---------------------------------------------------------------
:: Function: Download updated rdpwrap.ini file using pwsh
:: ---------------------------------------------------------------
:download_ini
echo [*] Attempting to download the latest rdpwrap.ini file...

:: 尝试下载第一个链接
set "rdpwrap_ini_url=%rdpwrap_ini_update_github_1%"
echo [*] Trying to download from %rdpwrap_ini_url%...
pwsh -Command "try { Invoke-WebRequest -Uri '%rdpwrap_ini_url%' -OutFile '%rdpwrap_ini_check%'; exit 0 } catch { exit 1 }"

if exist %rdpwrap_ini_check% (
    echo [+] Successfully downloaded the updated ini file from source 1.
    goto :eof
)

:: 如果第一个链接失败，尝试第二个链接
echo [-] Failed to download from source 1. Trying the second source...
set "rdpwrap_ini_url=%rdpwrap_ini_update_github_2%"
echo [*] Trying to download from %rdpwrap_ini_url%...
pwsh -Command "try { Invoke-WebRequest -Uri '%rdpwrap_ini_url%' -OutFile '%rdpwrap_ini_check%'; exit 0 } catch { exit 1 }"

if exist %rdpwrap_ini_check% (
    echo [+] Successfully downloaded the updated ini file from source 2.
    goto :eof
)

:: 如果两个链接都失败，输出错误信息
echo [-] Failed to download the updated ini file from both sources.
pause
goto :finish
::
:: -----------------------------------------------------
